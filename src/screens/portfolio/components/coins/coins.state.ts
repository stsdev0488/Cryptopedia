...
export const useCoinsState = () => {
  const [coins] = usePromise(
    async () => await trackPromise(getCoins(MAX_COINS))
  );
  const [coinList] = usePromise(async () => await getCoinList());

  const { transactions } = useReduxSelector((redux) => redux.transaction);

  const coinInfo = Object.entries(transactions).map(([symbol, number]) => {
